...
class List extends Component {
    ...
    componentDidMount=async()=>{
        let response= await fetch(`http://127.0.0.1:3457/students?limit=${this.state.pagesize}&offset=${this.state.page*this.state.pagesize}`,{
     ...
     addStudent=async()=>{
        let response= await fetch("http://127.0.0.1:3457/students",{
            headers: new Headers({'content-type': 'application/json'})
        })
        let students = await response.json()
     }
    ...
    saveNewStudent=async()=>{
      ...
      if(this.state.editmode===true){
        let response= await fetch("http://127.0.0.1:3457/students/"+this.state.student._id,{
          method:'PUT',
          body: JSON.stringify(this.state.student),
          headers: new Headers({'content-type': 'application/json'})
      })
    ...
    deleteStudent=async(id)=>{
      let response= await fetch("http://127.0.0.1:3457/students/"+id,{
        method:'DELETE',
        // body: JSON.stringify(this.state.student),
        headers: new Headers({'content-type': 'application/json'})
    }
        ...
        return (
          ...
          <Modal show={this.state.showModal}  onHide={this.handleClose}  >
            ...
            <>
             ...
             <Form.Control type="text" placeholder="Enter name" id='firstname'onChange={this.catchInput} />
         ...
         {this.state.editmode===true&&
            <>
             ...
             <Form.Group >
             <Form.Label>Name</Form.Label>
             <Form.Control type="text"  value={this.state.student.firstname} id='firstname'onChange={this.catchInput} />
           ...
           <Modal.Footer>
           <Button variant="primary" onClick={this.saveNewStudent}>Save</Button>
         </Modal.Footer>
         </>
         }
